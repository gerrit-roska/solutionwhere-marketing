...
//
// Fleet rule: without vendor keys, both sources run through Graphed Tools
// (`serper:search` / `exa:search`) so usage is metered and no personal keys
// exist. Raw SERPER_API_KEY / EXA_API_KEY remain as explicit overrides only.
...
async function fetchSerper(apiKey: string, keyword: string): Promise<string[]> {
  ...
  return serperLines((await response.json()) as SerperResponse);
}
...
async function fetchExa(apiKey: string, keyword: string): Promise<string[]> {
  ...
  return exaLines((await response.json()) as ExaResponse);
}

function serperLines(data: SerperResponse): string[] {
  const lines: string[] = [];
...
function exaLines(data: ExaResponse): string[] {
  const lines: string[] = [];
...
/** SERP research through Graphed Tools — the default, key-free path. */
async function fetchViaGraphedTools(keyword: string): Promise<string[]> {
  const graphed = new Graphed();
  if (!graphed.isConfigured()) return [];
  const sections: string[] = [];
  try {
    const serper = (await graphed.tools.run(
      "serper:search",
      { q: keyword, num: 10 },
      { timeoutSeconds: 120 },
    )) as SerperResponse;
    sections.push(...serperLines(serper));
  } catch (error) {
    console.warn(
      `serper:search failed, continuing without it: ${error instanceof Error ? error.message : error}`,
    );
  }
  try {
    const exa = (await graphed.tools.run(
      "exa:search",
      { query: keyword, numResults: 5 },
      { timeoutSeconds: 120 },
    )) as ExaResponse;
    sections.push(...exaLines(exa));
  } catch (error) {
    console.warn(
      `exa:search failed, continuing without it: ${error instanceof Error ? error.message : error}`,
    );
  }
  return sections;
}
...
export async function buildResearchBrief(
  ...
  // No vendor keys (the normal case): metered search through Graphed Tools.
  if (sections.length === 0) {
    sections.push(...(await fetchViaGraphedTools(keyword)));
  }

  if (sections.length === 0) {
    console.log(
      "No research available (Graphed Tools unconfigured, no SERPER_API_KEY / EXA_API_KEY) — generating without SERP grounding.",
    );
    return null;
  }
